...
void qmgr_t::update_json(const char *str, vector_t v, cJSON *out_obj, bool &alarm)
{
    ...
    if (!arr) {
        wifi_util_error_print(WIFI_CTRL,"ERROR: Missing Score array for MAC %s\n", str);
        pthread_mutex_unlock(&m_json_lock);
        return;
    }

    if (v.m_num == 0) {
        wifi_util_error_print(WIFI_CTRL,"ERROR: vector_t has m_num=0 for MAC %s\n", str);
        pthread_mutex_unlock(&m_json_lock);
        return;
    }

    if (v.m_num > MAX_LEN) {
        wifi_util_error_print(WIFI_CTRL,"ERROR: Invalid m_num=%d (MAX_LEN=%d) for MAC %s\n", v.m_num, MAX_LEN, str);
        pthread_mutex_unlock(&m_json_lock);
        return;
    }
    ...
    trim_cjson_array(arr, MAX_HISTORY);
    if (m_should_report == false) {
        pthread_mutex_unlock(&m_json_lock);
        return;
    }
    
    m_should_report = false;
    pthread_mutex_unlock(&m_json_lock);
    return;
}
...
int qmgr_t::run()
{
    ...
    //Temporary code
    strcpy(m_args.path, "/www/data");
    strcpy(m_args.output_file, "/www/data/telemetry.json");
    m_args.sampling =  5;
    m_args.reporting = 30;
    m_args.threshold = 0.4;
    long elapsed_sec  = 0;
 ...
 void qmgr_t::remove_device_from_out_obj(cJSON *out_obj, const char *mac_str)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (existing_mac && strcmp(existing_mac, mac_str) == 0) {
            ...
            wifi_util_info_print(WIFI_CTRL,"Removed device %s from out_obj\n", mac_str);
            return;
        }
    }
}
...
int qmgr_t::init(stats_arg_t *stats, bool create_flag)
{
    ...
    if (!create_flag) {
        if (device_exists) {
            wifi_util_info_print(WIFI_CTRL,"Removing device %s\n", mac_str);
            ...
            wifi_util_info_print(WIFI_CTRL,"Device %s not found, nothing to delete\n", mac_str);
        }
	 pthread_mutex_unlock(&m_json_lock);
        return 0;
    }
    ...
    // ---------- CREATE PATH ----------
    if (!device_exists) {
        wifi_util_info_print(WIFI_CTRL,"Adding new device %s\n", mac_str);
        cJSON_AddItemToArray(dev_arr, create_dev_template(mac_str));
    }

    linkq_t *lq = (linkq_t *)hash_map_get(m_link_map, mac_str);
    if (!lq) {
        lq = new linkq_t(mac_str);
        hash_map_put(m_link_map, strdup(mac_str), lq);
    }

    wifi_util_info_print(WIFI_CTRL,"Initializing linkq for %s\n", mac_str);
...
void qmgr_t::start_background_run()
{
   ...
   // if (!m_run_started) {
        ...
        if (ret != 0) {
            wifi_util_info_print(WIFI_CTRL,"Failed to create background run thread\n");
        } else {
           pthread_detach(tid);  // detach to run independently
            wifi_util_info_print(WIFI_CTRL,"created background run thread\n");
        }
 //   }
    wifi_util_info_print(WIFI_CTRL,"%s:%d\n",__func__,__LINE__);
}
...
char *qmgr_t::get_local_time(char *str, unsigned int len, bool hourformat)
{
    ...
    if(hourformat)
        strftime(str, len, "%H:%M:%S", local_time);
    else
