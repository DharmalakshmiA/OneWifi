...
web_t* web_t::get_instance(const char *path)
{
    ...
    wifi_util_info_print(WIFI_CTRL,"Pramod %s:%d\n",__func__,__LINE__);
...
char* web_t::find_token(char line[], const char symbol[], const char match[])
{
    ...
    while (token != NULL) {
        ...
        //wifi_util_error_print(WIFI_CTRL,"--Token: %s \n", token);
...
int web_t::send_message(int fd, char image_path[], char head[])
{
    ...
    if((fdimg = open(image_path, O_RDONLY)) < 0){
        wifi_util_error_print(WIFI_CTRL,"%s:%d: Cannot Open file in path : %s with error %d\n", __func__, __LINE__, image_path, fdimg);
        return -1;
    }
    ...
    while (offset < total_size) {
        ...
        if (sendfile(fd, fdimg, &offset, len) != 0) {
            //wifi_util_error_print(WIFI_CTRL,"%s:%d: send file error:%d\n", __func__, __LINE__, errno);
            close(fdimg);
            return -1;
        }
        offset += len;
...
int web_t::server(int sock)
{
    ...
    //wifi_util_error_print(WIFI_CTRL,"Client method: %s\n", parse_string_method);
    ...
    //wifi_util_error_print(WIFI_CTRL,"Client ask for path: %s\n", parse_string);
    ...
    if(parse_string_method[0] == 'G' && parse_string_method[1] == 'E' && parse_string_method[2] == 'T'){
        ...
        if ((find_string = find_token(buffer, "\r\n", "action")) != NULL) {
            ...
            wifi_util_error_print(WIFI_CTRL,"find string: %s \n", find_string);
   ...
   size_t stack_size = 1024 * 1024; // Example: 1 MB stack size
    ...
    while (m_exit == false) {
        if ((new_sock = accept(m_server_fd, (struct sockaddr *)&m_address, (socklen_t*)&addrlen))<0) {
            wifi_util_error_print(WIFI_CTRL,"%s:%d: Error in accept: %d\n", __func__, __LINE__, errno);
            return -1;
        }
        ...
        if (pthread_create(&stid, NULL, server, args) != 0) {
            wifi_util_error_print(WIFI_CTRL,"%s:%d: Failed to start child thread\n", __func__, __LINE__);
            close(new_sock);
            free(args);
			return -1;
        }
...
int web_t::start()
{
    ...
    if (init() != 0) {
        wifi_util_error_print(WIFI_CTRL,"%s:%d: Error in initializing server\n", __func__, __LINE__);
        return -1;
    }
    
    // Creating socket file descriptor
    if ((m_server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        wifi_util_error_print(WIFI_CTRL,"%s:%d: Error in cfreating socket: %d\n", __func__, __LINE__, errno);
        return -1;
    }

    if (bind(m_server_fd, (struct sockaddr *)&m_address, sizeof(m_address))<0) {
        wifi_util_error_print(WIFI_CTRL,"%s:%d: Error in binding: %d\n", __func__, __LINE__, errno);
        close(m_server_fd);
        return -1;
    }
    
    if (listen(m_server_fd, 10) < 0) {
        wifi_util_error_print(WIFI_CTRL,"%s:%d: Error in listen: %d\n", __func__, __LINE__, errno);
        return -1;
    }
