...
int process_ext_sta_conn_status(vap_svc_t *svc, void *arg)
{
    ...
    if (sta_data->stats.connect_status == wifi_connection_status_connected) {
        if ((ext->conn_state == connection_state_connection_in_progress) ||
            ...
            if (ctrl->rf_status_down == true) { 
                ...
                if (bssid_mac_str != NULL) {
                    ...
                    apps_mgr_link_quality_event(&ctrl->apps_mgr, wifi_event_type_exec, wifi_event_exec_register_station, bssid_mac_str, MAC_ADDR_STR_LEN);
                }
            }
        ...
        if (bssid_mac_str != NULL) {
            ...
            apps_mgr_link_quality_event(&ctrl->apps_mgr, wifi_event_type_exec, wifi_event_exec_unregister_station, bssid_mac_str, MAC_ADDR_STR_LEN);
        }
