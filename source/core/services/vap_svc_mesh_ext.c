...
int sort_bss_results_by_ranking(bss_candidate_t *scan_list, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (chan_util > IGNITE_MAX_CHUTIL_THRESHOLD) {
            wifi_util_dbg_print(WIFI_CTRL, "  -> CU %.2f ignored (above %.0f)\n", (float)scan_list[i].external_ap.chan_utilization, IGNITE_MAX_CHUTIL_THRESHOLD);
            continue;
        }
        ...
        *scores[valid_count].candidate = scan_list[i];
        scores[valid_count].bucket = (chan_util < IGNITE_MIN_CHUTIL_THRESHOLD) ? 1 : 2;
    ...
    for (int i = 0; i < valid_count; i++) {
        ...
        if (scores[i].bucket == 2) {
             ...
             if (snr_diff > IGNITE_SNR_DIFFERENCE) {
