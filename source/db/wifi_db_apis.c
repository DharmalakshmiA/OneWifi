...
#define ONEWIFI_DB_VERSION_ROGUE_AP_FLAG 100051
...
void callback_Wifi_Rogue_Config(ovsdb_update_monitor_t *mon,
    struct schema_Wifi_Rogue_Config *old_rec,
    struct schema_Wifi_Rogue_Config *new_rec)
{
    wifi_mgr_t *g_wifidb;
    g_wifidb = get_wifimgr_obj();
    ...
    if (mon->mon_type == OVSDB_UPDATE_DEL) {
        ...
        wifi_util_dbg_print(WIFI_DB,
            "%s:%d: Rogue Config New/Modify\n",
            __func__, __LINE__);

        if (new_rec == NULL) {
            wifi_util_dbg_print(WIFI_DB,
                "%s:%d: Null pointer Rogue config update failed\n",
                __func__, __LINE__);
            return;
        }
                                       ...
                                       bool add)
{
    ...
    if (!add) {
        ...
        if (ret < 0) {
            wifidb_print("%s:%d DB delete failed vap_name=%s ret=%d\n",
    ...
    for (int s = 0; s < MAX_KNOWN_APS; s++) {
        ...
        wifi_util_dbg_print(WIFI_DB,
                            "%s:%d   mac_list[%d]=%s slot=%d vap_name=%s\n",
                            __func__, __LINE__,
    ...
    wifi_util_dbg_print(WIFI_DB,
                        "%s:%d upserting vap_name=%s mac_list_len=%d\n",
                        __func__, __LINE__, vap_name, cfg.mac_list_len);

    /* ---- Check if row already exists --------------------------- */
    where = onewifi_ovsdb_tran_cond(OCLM_STR, "vap_name",
                                    OFUNC_EQ, vap_name);
    struct schema_Wifi_KnownAp_Config *pcfg =
        onewifi_ovsdb_table_select_where(g_wifidb->wifidb_sock_path,
                                         &table_Wifi_KnownAp_Config,
                                         where, &ret);
    if (where) { json_decref(where); where = NULL; }
    ...
    if (update) {
        ...
        if (ret <= 0) {
            wifidb_print("%s:%d DB update failed vap_name=%s ret=%d\n",
                         __func__, __LINE__, vap_name, ret);
            return RETURN_ERR;
        }
        wifidb_print("%s:%d updated row vap_name=%s mac_list_len=%d\n",
                     __func__, __LINE__, vap_name, cfg.mac_list_len);
    ...
    /* ---- INSERT new row ---------------------------------------- */
        ...
        if (!onewifi_ovsdb_table_insert(g_wifidb->wifidb_sock_path,
                                         &table_Wifi_KnownAp_Config,
                                         &cfg)) {
            wifidb_print("%s:%d DB insert failed vap_name=%s\n",
                         __func__, __LINE__, vap_name);
            return RETURN_ERR;
        }
        wifidb_print("%s:%d inserted row vap_name=%s mac_list_len=%d\n",
                     __func__, __LINE__, vap_name, cfg.mac_list_len);
    }
...
int wifidb_update_wifi_rogue_config(wifi_RogueConfig_t *config)
{
    struct schema_Wifi_Rogue_Config cfg;

    memset(&cfg, 0, sizeof(cfg));

    if (config == NULL)
    {
        wifidb_print("%s:%d WIFI DB update error !!!. Failed to update Rogue Config table \n", __func__, __LINE__);
        return -1;
    }
    ...
    {
        wifidb_print("%s:%d Updated WIFI DB. Rogue Config table updated successful.\n", __func__, __LINE__);
    }
...
void wifidb_init_rogue_config_default(wifi_RogueConfig_t *config)
{
      wifi_RogueConfig_t rogue_config;
      wifi_mgr_t *g_wifidb;
      g_wifidb = get_wifimgr_obj();
...
int update_wifi_rogue_config(wifi_RogueConfig_t *config)
{
    int ret = 0;
    ...
    wifidb_print("%s:%d WIFI DB update error !!!. Failed to update Rogue Config\n",__func__, __LINE__);
    return -1;
}
...
int start_wifidb_monitor()
{
    ...
    ONEWIFI_OVSDB_TABLE_MONITOR(g_wifidb->wifidb_fd, Wifi_Rogue_Config, true);
