...
int validate_assoc_client_args(wifi_mon_stats_args_t *args)
{
    ...
    wifi_util_error_print(WIFI_MON, "%s:%d [DL] Ignite status : %d\n", __func__, __LINE__, ctrl->rf_status_down);
    ...
    unsigned long task_interval_ms)
{
    ...
    if (link_data != NULL) {
        wifi_util_dbg_print(WIFI_CTRL, "%s:%d: diag result: number of devs: %d\n", __func__, __LINE__,
         num_devs);
   }
    for (i = 0; i < num_devs; i++) {
        wifi_util_dbg_print(WIFI_MON,
			...
			wifi_util_info_print(WIFI_CTRL,"cli_SNR: %d,cli_PacketsSent: %lu,cli_ErrorsSent: %lu,cli_LastDataDownlinkRate: %d  per = %.2f\n",
			dev_array[i].cli_SNR,dev_array[i].cli_PacketsSent,dev_array[i].cli_ErrorsSent,dev_array[i].cli_LastDataDownlinkRate,link_data[i].stats.per);


	} 
    if(num_devs != 0) {
        //wifi_util_info_print(WIFI_CTRL,"sending the events to link quailty\n");
        apps_mgr_link_quality_event(&ctrl->apps_mgr, wifi_event_type_exec, wifi_event_exec_timeout, link_data, num_devs);
	}
    if(link_data) {
        //wifi_util_info_print(WIFI_CTRL,"Freeing link quailty\n");
		free(link_data);
	}
    ...
    while (sta != NULL) {
        ...
        if (tmp_sta != NULL) {
            ...
            if (link_data != NULL) {
                ...
                wifi_util_dbg_print(WIFI_CTRL, "%s:%d: diag client disassociated  sta mac=%s:\n", __func__, __LINE__,link_data->stats.mac_str);
            }
